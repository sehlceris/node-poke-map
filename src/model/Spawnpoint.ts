...
export default class Spawnpoint {
    ...
    startSpawnTimer():void {
        ...
        this.timerStartTimeout = setTimeout(() => {
            ...
            this.accurateInterval = new AccurateInterval(() => {
                this.fireSpawn();
            }, Utils.timestepTransformDown(Constants.HOUR), true);
        }, Utils.timestepTransformDown(firstFireDelay));

        // let spawnTimeout = this.startTime - new Date(); //time diff from now that spawn will occur
    ...
    stopSpawnTimer():void {
        ...
        if (this.accurateInterval) {
            this.accurateInterval.clear();
        }
    }
